...
### 子节点更新
- 新增节点。newChildren里有，oldChildren没有，那么新节点插在真实的**未处理节点**之前。
- 更新节点。新旧子节点都有，且位置一样。
- 移动节点。新旧子节点都有，且位置不一样。那么新节点需要移动到真实的**未处理节点**之前。
- 删除节点。旧子节点未被处理的节点即视为废弃节点。

#### 优化策略
在对一个新子节点做操作时，需要遍历旧子节点查询之前，可以进行4种快速对比，来看看是否能找到该子节点。分别是：
- 新前与旧前
- 新后与旧后
- 新后与旧前
- 新前与旧后
...
静态节点概念：
- 不带变量的纯文本节点
- 元素节点稍复杂，比如不能使用动态绑定语法v-、@、:开头属性，不能使用v-for、v-if、v-else指令，不能是内置标签，不能是组件等等。
